...
type DynArrInt = packed array of integer;
    Mass = array [1..255] of integer;
...
var res_sign: boolean = true;
    ...
    fin: boolean = false;
    epsilon: double;
    arrayOfAnsBases: Mass;

procedure mainFinish(res_sign: boolean; res: double; epsilon: double; var arrayOfAnsBases: Mass); forward;

{called in case of an error, outputs the last result received}
procedure finByMistake(res: double; epsilon: double; var arr: Mass);
begin
...
procedure mainReadInit(var accurasy:double; var A: Mass);
var kol, i :integer; 
begin
    kol:=ParamCount();
    if kol < 2 then
    ...
    begin
        writeln('Incorrect value of epsilon. Epsilon must be bigger than 0 and lower than 1');
        halt(1);
    end;
    for i:=2 to kol do
    begin
...
begin
    ...
    repeat
        ...
        {the condition for skipping all spaces and tabs and enters in the end of line}
        if ((ord(c) = ord(' ')) or (ord(c) = 9) or (ord(c) = 10)) then
            continue;
...
procedure mainFinish(res_sign: boolean; res: double; epsilon: double; var arrayOfAnsBases: Mass);
var
    integerPartRes, fractionalPartRes: double;
    i: int64;
begin
    i:=1;
    fractionalPartRes := abs(res) - abs((int(res)));
    integerPartRes := abs(int(res));
    {output with formatting}
    while arrayOfAnsBases[i] <> 0 do
    begin
        if (arrayOfAnsBases[i + 1] <= 9) then
        begin
            ...
            write(' . ');
            transferToCustomAfterDot(arrayOfAnsBases[i + 1], epsilon, fractionalPartRes);
            writeln;
        end;
        if ((arrayOfAnsBases[i + 1] >= 10) and (arrayOfAnsBases[i + 1] <= 99)) then
        begin
            write(arrayOfAnsBases[i + 1], '    ');
            if (res_sign = false) then
                write('-');
            transferToCustom(integerPartRes, arrayOfAnsBases[i + 1]);
            write(' . ');
            transferToCustomAfterDot(arrayOfAnsBases[i + 1], epsilon, fractionalPartRes);
            writeln;
        end;
    ...
    begin
        ...
        begin
            case arg_operation of
                '+': begin
                    if (res >= exp(40 * LN(10)) - num) or (res >= exp(40 * LN(10))) then
                    begin
                        writeln('Input Error, overflow when entering, too large number is entered');
                        finByMistake(res, epsilon, arrayOfAnsBases);
                    end;
