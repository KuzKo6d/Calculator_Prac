...
{checking for overflow and do multiplication}
function subMultiplicate(res, arg: double): double;
begin
  {if arg = 0 then chtcking for overflow is incorrect}
  if (arg=0) then
    begin
      res:=0;
      res_sign:=true;
...
{checking for overflow and do division}
function subDivision(res, arg: double): double;
begin
  {if arg = 0 then math problem}
  if (arg = 0) then
    begin
...
{checking for 16-system}
function checking_16(c: char): boolean;
begin
  checking_16 := false;
...
procedure mistake_finish(result: double; accuracy: double; var arr: start_args);
begin
...
procedure to_system(base: integer; res: longword);
var
  new_res: longword;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    mistake_finish(result, accuracy, out_base);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          i := i + 1;
          read(d);
          if (checking_16(c) and checking_16(d)) then
          begin
            if ((ord(c) >= ord('0')) and (ord(c) <= ord('9'))) then
